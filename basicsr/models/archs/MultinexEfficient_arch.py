...
class MDTA(nn.Module):
    def __init__(self, dim, num_heads, bias=True):
        ...
        # self.qkv = DWSeparableConv(dim, dim*3, k=3)
        # self.project_out = DWSeparableConv(dim, dim, k=3)
...
class MultinexEfficient(nn.Module):
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = F.interpolate(input=out, scale_factor=2)
